...
- Aplica erosión morfológica al bosque (evitar pixeles de borde)
...
Ejemplo local (para testing):
  python 04_rasterizar_labels.py \\
    ...
    parser.add_argument("--erosion_pixels", type=int, default=1,
                       help="Pixeles de erosión para el bosque (default=1)")
    ...
    if not all(p.startswith('s3://') for p in [args.ref, args.bosque_shp, args.output]):
        print(f"\n[ERROR] Error: Paths deben ser S3 (s3://...)")
        sys.exit(1)
    
    if args.perimetro_shp and not args.perimetro_shp.startswith('s3://'):
        print(f"\n[ERROR] Error: Path de perímetro debe ser S3 (s3://...)")
        sys.exit(1)
    ...
    print(f"   Erosión: {args.erosion_pixels} pixeles")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(f"\n  Descargando raster de referencia...")
        ...
        print(f"   [OK] Dimensiones: {width} × {height} px")
        print(f"   [OK] CRS: {crs}")
        ...
        print(f"   [OK] Bosque: {len(gdf_bos)} polígonos (CRS: {gdf_bos.crs})")
        
        if args.perimetro_shp:
            local_per_shp = s3_handler.download_shapefile(args.perimetro_shp, tmpdir)
            gdf_per = gpd.read_file(local_per_shp)
            print(f"   [OK] Perímetro: {len(gdf_per)} polígonos (CRS: {gdf_per.crs})")
        else:
            gdf_per = None
        ...
        print(f"   [OK] Reproyección completada")
        ...
        print(f"   [OK] Bosque unificado: {bosque_union.geom_type}")
        if perimetro_union:
            print(f"   [OK] Perímetro unificado: {perimetro_union.geom_type}")
        ...
        print(f"   [OK] Pixeles de bosque (pre-erosión): {n_bosque_pre:,} ({n_bosque_pre/(height*width)*100:.2f}%)")
        
        # --- 6) Aplicar erosión morfológica
        erosion_px = max(0, int(args.erosion_pixels))
        if erosion_px > 0:
            print(f"\n  Aplicando erosión morfológica ({erosion_px} pixeles)...")
            ...
            print(f"   [OK] Pixeles de bosque (post-erosión): {n_bosque_post:,} ({n_bosque_post/(height*width)*100:.2f}%)")
            print(f"   [OK] Pixeles removidos: {n_removed:,} ({n_removed/n_bosque_pre*100:.1f}%)")
        else:
            print(f"\n Sin erosión (erosion_pixels=0)")
            n_bosque_post = n_bosque_pre
        
        # --- 7) Construir etiqueta final: -1 ignorar, 0 no-bosque, 1 bosque
        print(f"\n  Construyendo etiquetas finales...")
        ...
        if perimetro_union is not None:
            ...
            print(f"   [OK] Pixeles dentro del perímetro: {n_perimetro:,} ({n_perimetro/(height*width)*100:.2f}%)")
            ...
            # Sin perímetro: toda la malla es area válida
            label[:, :] = 0
            label[bosque_r == 1] = 1
            print(f"    Sin perímetro: toda la malla es area válida")
        ...
        print(f"   Bosque (1):      {n_bosque:,} pixeles ({n_bosque/total*100:.2f}%)")
        print(f"   No-Bosque (0):   {n_no_bosque:,} pixeles ({n_no_bosque/total*100:.2f}%)")
        print(f"   Ignorar (-1):    {n_ignorar:,} pixeles ({n_ignorar/total*100:.2f}%)")
        print(f"   Total:           {total:,} pixeles")
        
        if n_bosque > 0 and n_no_bosque > 0:
            ratio = n_no_bosque / n_bosque
            print(f"\n   Ratio No-Bosque/Bosque: {ratio:.2f}:1")
            if ratio > 10:
                print(f"   [WARN]  Advertencia: Desbalance significativo (ratio > 10:1)")
            elif ratio > 3:
                print(f"   [WARN]  Desbalance moderado (ratio > 3:1)")
            else:
                print(f"   [OK] Balance razonable de clases")
        ...
        print(f"   [OK] Formato: GeoTIFF int16, nodata=-1")
    
    # Resumen final
    print(f"\n" + "="*70)
    print("[OK] RASTERIZACIÓN COMPLETADA")
    print("="*70)
    
    print(f"\nLeyenda del raster generado:")
    print(f"   1  = Bosque (positivo, erosionado {erosion_px}px)")
    print(f"   0  = No-Bosque (negativo, dentro del area válida)")
