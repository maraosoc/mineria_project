...
Multiple studies over the years, summarized by [1], have found and agree that in Colombia, the leading causes of deforestation on a national level are the expansion of the agricultural frontier (both for legal and illegal usage of the land) and the intensive logging activities for both commercial and domestic purposes. The studies also conclude that, since Colombia is a highly diverse country, the country’s regions all possess different biophysical characteristics and are affected by distinctive phenomena. Consequently, it is worth studying the dynamics of deforestation on a subnational or subregional level to obtain more accurate and focused results [1].

Las imágenes satelitales son ampliamente usadas junto a sistemas de información geográfica para el mapeo geográfico y la analítica en diversos sectores de la ciencia y la industria. En particular, existen métodos de machine learning implementados en campos como la ecología forestal y existen diversas aplicaciones en áreas como la distribución de especies, ciclos de carbono, gestión de riesgo y pronóstico. Uno de los insumos de gran utilidad es la distribución de áreas forestales en cierta región y esta información puede ser extraída a partir de imágenes satelitales.
...
```{mermaid}
%%{init: {'flowchart': {'curve':'basis','rankSpacing':90,'nodeSpacing':60}} }%%
flowchart LR

%% ======== SECCIONES PRINCIPALES ========
subgraph LOCAL["💻 Entorno Local"]
    L1["1️⃣ Subida de shapes a S3
    - Carga shapefiles y/o listas de imágenes objetivo
    - Organización en:
      s3://<bucket>/shapes/
      s3://<bucket>/lists/imagenes_objetivo.csv"]
    
    L2["2️⃣ Descarga de datos Sentinel-2
    - Búsqueda y descarga histórica de imágenes
...
direction LR
  ...
  subgraph EC2["🖥️ EC2 - Preprocesamiento"]
    A3["3️⃣ 01_procesar_sentinel.py
    - Procesa bandas e índices
    - Reproyección al CRS objetivo"]

    A4["4️⃣ 02_generar_mascaras.py
    - Máscaras cielo despejado
    - NDVI/NIR y dilatación"]

    A5["5️⃣ 03_tabular_features.py
    - Extrae píxeles válidos
    - Estadísticas temporales"]

    A6["6️⃣ 04_rasterizar_labels.py
    - Rasteriza bosque desde shapes
    - Erosión morfológica"]

    A7["7️⃣ 05_unir_features_labels.py
  ...
  %% EMR Entrenamiento
  subgraph EMR["🔥 EMR - Entrenamiento y Evaluación"]
    A8["8️⃣ 06_entrenar_modelos_spark.py
    - Entrena RandomForest o GBT en Spark (EMR)"]

    A9["9️⃣ 07_evaluar_modelos.py
    - Calcula métricas y genera reporte"]
...
EXIT_AWS["🚪 Salida hacia S3"]

ENTRY_AWS --> A3
A7 -->|Dataset final a S3: features y labels| A8
A9 --> EXIT_AWS
EXIT_AWS -->|Modelos y métricas| S3
